...
export class CityStaffHomeGuard implements CanActivate {
    ...
    return this._auth.getCurrentUser().pipe(
      map((data) => {
        if (data?.attributes['custom:access_group'] === 'admin') {
          return true;
        } else if (data?.attributes['custom:access_group'] === 'city_staff') {
          return true;
        } else if (
          data?.attributes['custom:access_group'] === 'city_staff_guest'
        ) {
          return true;
        } else {
