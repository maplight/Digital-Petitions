describe('Inactive Petitions Test', () => {
  beforeEach(() => {
    cy.visit('/inactive-petitions');
  });
    ...
    cy.intercept(
      ...
      (req: any) => {
        ...
        if (req.body.query.search('query GetPetitionsByType') == 0) {
          req.reply({ fixture: 'inactive-petitions.json' });
        }
      }
    );

    cy.get('h1', { timeout: 10000 }).should('have.text', 'Inactive Petitions');
  });
  ...
  it('should show six mat-chips elements in the filters when the view corresponding a large screen device', () => {
    ...
    cy.get('mat-chip', { timeout: 10000 }).should(($element) => {
      expect($element).to.have.length(12);
    });
  });
  ...
  it('should show details of petition when the title of petition is clicked', () => {
    cy.intercept(
      'POST',
      'https://neouilttfjcc5ght2tbr7of7mu.appsync-api.us-east-1.amazonaws.com/graphql',
      (req: any) => {
    ...
    cy.get('h3', { timeout: 10000 }).should(($h3) => {
      $h3[1].click();
    });
